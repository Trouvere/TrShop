...
import com.trouvere.security.TokenAuthenticationEntryPoint;
import com.trouvere.security.TokenAuthenticationFilter;
import com.trouvere.security.TokenAuthenticationProvider;
...
public class SecurityConfig extends WebSecurityConfigurerAdapter {
	...
	protected void configure(AuthenticationManagerBuilder auth) throws Exception {
		auth.inMemoryAuthentication().withUser("user").password("password").roles("USER").and().withUser("admin")
				.password("adminPassword").roles("USER", "ADMIN").and().and()
				.authenticationProvider(tokenAuthenticationProvider(auth.getDefaultUserDetailsService()));
	}

	@Override
	protected void configure(HttpSecurity http) throws Exception {
		http.addFilterBefore(new TokenAuthenticationFilter(authenticationManager(), tokenAuthenticationEntryPoint(),
				header, ignoreFault), BasicAuthenticationFilter.class).sessionManagement()
